"""
Time evaluation for EMNIST and NDCE. 
"""
...
for fold, (train, test) in enumerate(rskf.split(dataset.targets, dataset.targets)):
    ...
    loss_fn = NonDecisiveCrossEntropyLoss(c=0, w=1, n_classes=10)
    ...
    loss_full[fold][:losses.shape[0]] = np.array(losses)
    scores_train[fold][:accuracies.shape[0]] = np.array(accuracies)
    scores_test[fold][:test_accuracies.shape[0]] = np.array(test_accuracies)  

    np.save(f"scores/exp_2/loss", loss_full)
    np.save(f"scores/exp_2/scores_train", scores_train)
    np.save(f"scores/exp_2/scores_test", scores_test)
    np.save(f"scores/exp_2/times_train", times_train)
    np.save(f"scores/exp_2/times_test", times_test)

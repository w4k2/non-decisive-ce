"""
Check different learning rates and batch sizes for all benchmarks - Cross Entropy. 
"""
...
for batch_idx, batch_size in enumerate(batch_sizes):
    for data_idx, data in enumerate(datasets):
        ...
        scores_train = np.zeros((len(learning_rates), n_splits * n_repeats, 
                                epochs * math.ceil(X_full.shape[0] / n_splits / batch_size)))
        scores_test = np.zeros((len(learning_rates), n_splits * n_repeats, 
                                epochs))

        # datasets x folds x epochs
        loss_full = np.zeros((len(learning_rates), n_splits * n_repeats, 
                            epochs * math.ceil(X_full.shape[0] / n_splits / batch_size)))

        # datasets x folds x epochs x probas
        ...
        for lr_idx, lr in enumerate(learning_rates):
            for fold, (train, test) in enumerate(rskf.split(X_full, y_full)):
                print(f"# BS {batch_size} -- DATASET {name} -- LR {lr} -- FOLD {fold}")
            ...
            np.save(f"scores_ce/exp_1/loss_{name}_{batch_size}", loss_full)
            np.save(f"scores_ce/exp_1/scores_train_{name}_{batch_size}", scores_train)
            np.save(f"scores_ce/exp_1/scores_test_{name}_{batch_size}", scores_test)
            np.save(f"scores_ce/exp_1/probas_train_{name}_{batch_size}", probas_train)
            np.save(f"scores_ce/exp_1/probas_test_{name}_{batch_size}", probas_test_full)
